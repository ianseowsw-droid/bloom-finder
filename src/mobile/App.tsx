import { Camera, CheckCircle2, ChevronRight, Crosshair, ImagePlus, Info, Leaf, MapPin, Search, Sparkles, Trophy, X } from "lucide-react";
import { useEffect, useMemo, useRef, useState } from "react";
import { createRoot } from "react-dom/client";
import "../styles.css";
import heroImg from "@/assets/hero-monstera.jpg";
import { addSpecimen, collectionStats, specimenTags, timeAgo, useCollection, type Specimen } from "@/lib/collection";
  ...
  return (
    <div className="mobile-viewport bg-sage font-sans text-forest">
      <div className="mobile-scroll mobile-content-bottom flex w-full flex-col bg-sage">
        ...
        {screen.name === "profile" && <ProfileScreen profile={profile} onProfileChange={setProfile} />}
    ...
    <>
      ...
      <main className="flex-1 px-4 py-2">
        <section className="mb-4 rounded-[1.75rem] border border-forest/5 bg-bone p-4 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <div>
              <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-gold">Today&apos;s hunt</span>
              <h2 className="mt-1 font-serif text-2xl font-bold leading-tight">Find, press, collect.</h2>
            </div>
        ...
        <div className="mt-6 grid grid-cols-3 gap-3">
          ...
          <SmallStat label="Latest" value={collection[0] ? timeAgo(collection[0].createdAt) : "-"} />
      ...
      <section className="mt-4 rounded-t-[3rem] bg-bone px-6 py-8 shadow-[0_-20px_40px_-15px_rgba(0,0,0,0.05)]">
        <div className="mb-6 flex items-end justify-between">
          <div>
            <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-gold">Collection</span>
        ...
        {recent.length === 0 ? (
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
            {recent.map((s) => (
              <button key={s.id} type="button" onClick={() => go({ name: "specimen", id: s.id })} className="w-36 flex-shrink-0 overflow-hidden rounded-2xl border border-forest/5 bg-sage text-left shadow-sm">
      ...
      <div className="mx-auto w-full max-w-md">
        <div className="rounded-[2rem] bg-forest p-6 text-bone shadow-2xl shadow-forest/20">
          ...
          <p className="mt-3 text-sm leading-relaxed text-bone/75">
            Bloom Finder creates a secure Supabase session, then saves your herbarium,
            specimens, and preferences to your account.
          </p>
        </div>
...
function Herbarium({ go }: { go: (screen: Screen) => void }) {
  ...
  const filtered = collection.filter((s) => {
    const haystack = [s.name, s.latin, s.notes, s.foundAt, s.habitat, s.color]
      ...
      .join(" ")
      .toLowerCase();
    return haystack.includes(query.trim().toLowerCase()) && (filter === "All" || specimenTags(s).includes(filter));
  });

  return (
    <>
      <PageHeader eyebrow="Collection" title="Specimen Album" copy={collection.length === 0 ? "Your album is empty. Start a plant hunt to earn your first card." : `${collection.length} collectible specimen card${collection.length === 1 ? "" : "s"} in your album.`} />
      <main className="flex-1 px-6">
        {collection.length === 0 ? (
          <button type="button" onClick={() => go({ name: "capture" })} className="block w-full rounded-3xl border-2 border-dashed border-moss/30 bg-bone/50 p-10 text-center">
            ...
            <p className="mt-1 text-xs font-semibold uppercase tracking-[0.2em] text-moss/60">Start encounter</p>
          </button>
        ) : (
          <>
            ...
            <section className="mt-5 rounded-2xl border border-forest/5 bg-bone p-4">
              <div className="flex items-center gap-2">
                ...
                <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search card, place, notes" className="min-w-0 flex-1 bg-transparent text-sm placeholder:text-moss/40 focus:outline-none" />
              </div>
              <div className="mt-4 flex gap-2 overflow-x-auto no-scrollbar">
                {filters.map((f) => (
                  <button key={f} type="button" onClick={() => setFilter(f)} className={"whitespace-nowrap rounded-full border px-3 py-2 text-[10px] font-bold uppercase tracking-wider " + (filter === f ? "border-forest bg-forest text-bone" : "border-forest/5 bg-sage text-moss")}>
                    {f}
                  </button>
                ))}
              </div>
            ...
            <div className="mt-6 grid grid-cols-2 gap-4">
              {filtered.map((s) => (
                <SpecimenCardButton key={s.id} specimen={s} onClick={() => go({ name: "specimen", id: s.id })} />
              ))}
            </div>
          </>
        )}
...
function Capture({ go }: { go: (screen: Screen) => void }) {
  ...
  const checks = image
    ...
    : [
        ...
        { title: "Frame the subject", detail: "Keep the plant inside the guide corners", done: false },
      ];
    ...
    <>
      <main className="min-h-dvh flex-1 bg-[#151812] px-4 pb-8 text-bone">
        <div className="mobile-safe-top flex items-center justify-between pb-4">
          <button onClick={() => go({ name: "field" })} className="grid size-10 place-items-center rounded-full border border-bone/20 bg-bone/90 text-forest shadow-lg" aria-label="Close capture">
            <X className="size-4" />
          </button>
          <div className="rounded-full border border-bone/15 bg-bone/10 px-4 py-2 text-center backdrop-blur-md">
            <span className="block text-[9px] font-semibold uppercase tracking-[0.18em] text-bone/55">Encounter</span>
            <span className="block text-sm font-bold">Plant hunt</span>
          </div>
          <button type="button" onClick={useGps} disabled={locating} className="grid size-10 place-items-center rounded-full border border-bone/20 bg-bone/90 text-forest shadow-lg disabled:opacity-60" aria-label="Use GPS">
          ...
          <div className="space-y-4">
            <section className="rounded-3xl border border-bone/15 bg-bone p-3 text-forest shadow-2xl">
              <div className="space-y-2">
                {checks.map((check) => (
                  <div key={check.title} className="flex items-center gap-3 rounded-2xl bg-sage px-3 py-3">
            ...
            <section className="relative aspect-[3/4] overflow-hidden rounded-[2rem] border border-bone/10 bg-gradient-to-b from-[#44523e] via-[#263326] to-[#0d160f] shadow-2xl">
              <img src={heroImg} alt="" className="absolute inset-0 h-full w-full object-cover opacity-55 blur-[1px]" />
              ...
              <div className="absolute left-6 right-6 top-16 rounded-3xl border border-bone/20 bg-bone/10 p-5 text-center backdrop-blur-md">
                <span className="text-[10px] font-bold uppercase tracking-[0.22em] text-gold">Live guide</span>
                <h2 className="mt-2 font-serif text-3xl font-bold italic leading-tight">Spot a bloom in the real world.</h2>
                <p className="mt-3 text-sm leading-relaxed text-bone/75">Open the camera, frame the plant, then turn it into a collectible specimen card.</p>
              </div>
              ...
              <div className="absolute bottom-5 left-5 rounded-2xl border border-bone/20 bg-bone px-4 py-3 text-forest shadow-xl">
                <span className="text-xs font-bold uppercase tracking-[0.18em] text-gold">3 / 3 tries</span>
              </div>
              <div className="absolute bottom-5 right-5 flex flex-col gap-2">
                <button onClick={() => fileRef.current?.click()} className="grid size-14 place-items-center rounded-full bg-gold text-forest shadow-xl active:scale-95" aria-label="Open camera">
                  <Camera className="size-6" strokeWidth={1.75} />
                </button>
                <button onClick={() => fileRef.current?.click()} className="grid size-11 place-items-center rounded-full border border-bone/20 bg-bone/90 text-forest shadow-lg active:scale-95" aria-label="Choose from library">
            ...
            <div className="rounded-3xl border border-bone/10 bg-bone/10 p-4 backdrop-blur-md">
              <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-bone/50">How capture works</span>
              <p className="mt-2 text-sm leading-relaxed text-bone/75">This build opens the native iPhone camera. The live AI guide and plant visibility checks can be connected next.</p>
            </div>
          </div>
        ) : (
          <div className="space-y-4">
            <section className="rounded-3xl border border-bone/15 bg-bone p-3 text-forest shadow-2xl">
              {checks.map((check) => (
                <div key={check.title} className="flex items-center gap-3 border-b border-forest/5 px-2 py-3 last:border-0">
            ...
            <div className="relative aspect-[3/4] overflow-hidden rounded-[2rem] border border-bone/10 bg-white shadow-2xl">
              ...
              <span className="absolute left-4 top-4 rounded-full border border-bone/20 bg-bone/90 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-forest backdrop-blur">Plant found</span>
              <div className="absolute inset-x-5 bottom-5 rounded-3xl border border-bone/20 bg-bone p-5 text-forest shadow-2xl">
                <span className="text-[10px] font-bold uppercase tracking-[0.22em] text-gold">Specimen card</span>
                <h2 className="mt-1 font-serif text-3xl font-bold italic leading-tight">{name.trim() || "Unnamed bloom"}</h2>
                <p className="mt-2 text-xs leading-relaxed text-moss/75">Rarity, confidence, habitat, and location become part of this collectible once saved.</p>
                <div className="mt-4 flex items-center justify-between">
                  <span className="rounded-full bg-sage px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-moss">#FIELD-{String(Date.now()).slice(-4)}</span>
                  <button type="button" onClick={() => setImage(null)} className="text-[10px] font-bold uppercase tracking-wider text-gold">Retake</button>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 rounded-[2rem] bg-sage p-4 text-forest">
              <div className="rounded-2xl border border-forest/5 bg-bone p-4 shadow-sm">
                <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-moss/60">Storage</span>
                <p className="mt-2 text-sm leading-relaxed text-moss/80">
                  {isSupabaseConfigured() ? "This specimen saves locally first, then syncs to Supabase." : "This specimen saves locally until Supabase is configured."}
                </p>
              </div>
              <Input label="Give it a name" value={name} onChange={setName} placeholder="e.g. Cottage Rose" serif />
              <Input label="Latin name" value={latin} onChange={setLatin} placeholder="e.g. Rosa gallica" serif />
              <div className="grid grid-cols-2 gap-3">
                <Select label="Color" value={color} onChange={setColor} options={["White", "Yellow", "Pink", "Red", "Blue", "Purple", "Green"]} />
                <Select label="Habitat" value={habitat} onChange={setHabitat} options={["Garden", "Meadow", "Forest", "Roadside", "Wetland", "Indoor"]} />
              </div>
              <label className="block">
                <span className="flex items-center justify-between gap-3">
                  <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-moss/60">Found at</span>
                  <button type="button" onClick={useGps} disabled={locating} className="inline-flex items-center gap-1.5 rounded-full bg-sage px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-forest disabled:opacity-50">
                    <Crosshair className="size-3" strokeWidth={1.75} />
                    {locating ? "Locating" : "Use GPS"}
                  </button>
                </span>
                <input value={foundAt} onChange={(e) => setFoundAt(e.target.value)} placeholder="e.g. North path, Botanic Garden" className="mt-2 w-full rounded-2xl border border-forest/10 bg-bone px-4 py-4 text-sm placeholder:text-moss/40 focus:border-gold focus:outline-none" />
                {fieldLocation && <span className="mt-2 flex items-center gap-1.5 text-[11px] text-moss/70"><MapPin className="size-3" strokeWidth={1.75} />GPS saved, accurate to about {Math.round(fieldLocation.accuracy)}m.</span>}
                {locationError && <span className="mt-2 block text-[11px] text-red-700">{locationError}</span>}
              </label>
              <label className="block">
                <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-moss/60">Field notes</span>
                <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} placeholder="Where you found it, the light, the scent..." className="mt-2 w-full resize-none rounded-2xl border border-forest/10 bg-bone px-4 py-3 text-sm leading-relaxed placeholder:text-moss/40 focus:border-gold focus:outline-none" />
              </label>
              <div className="grid grid-cols-[auto_1fr] gap-3 pt-2">
                <button onClick={() => setImage(null)} className="rounded-2xl border border-forest/5 bg-bone px-5 text-xs font-semibold uppercase tracking-wider text-forest">Retake</button>
                <button onClick={save} disabled={!name.trim()} className="rounded-2xl bg-forest py-5 text-sm font-semibold uppercase tracking-[0.15em] text-bone shadow-lg shadow-forest/20 disabled:opacity-40">Add to Collection</button>
              </div>
            </div>
          </div>
        )}
        <input ref={fileRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) void onFile(f); e.target.value = ""; }} />
      </main>
    </>
  );
}
...
function Discover({ go }: { go: (screen: Screen) => void }) {
  ...
  const quests = [
    { title: "Press your first specimen", detail: "Capture any plant and save it to your herbarium.", progress: Math.min(collection.length, 1), total: 1 },
    { title: "Find 3 bloom colors", detail: "Add color data while saving specimens.", progress: Math.min(colorCount, 3), total: 3 },
    { title: "Record 2 habitats", detail: "Save plants from different places.", progress: Math.min(habitatCount, 2), total: 2 },
  ];

  return (
    <>
      <PageHeader eyebrow="Field guide" title="Discover" copy="Browse plants that may be nearby, then tap a card to learn what to look for." />
      <main className="flex-1 space-y-5 px-6">
        <section className="rounded-3xl border border-forest/5 bg-bone p-5 shadow-sm">
          <div className="flex items-start gap-3">
            <span className="grid size-10 shrink-0 place-items-center rounded-2xl bg-sage text-forest"><Info className="size-5" strokeWidth={1.75} /></span>
            <div>
              <h2 className="font-serif text-xl font-bold">How this page works</h2>
              <p className="mt-1 text-sm leading-relaxed text-moss/80">These are sample plant sightings for the current app build. Your GPS-to-region matching can plug into this list later.</p>
            </div>
          </div>
        </section>
        <section className="grid grid-cols-2 gap-3">
          <button onClick={() => go({ name: "capture" })} className="rounded-2xl bg-forest p-4 text-left text-bone shadow-lg shadow-forest/10">
            <Camera className="size-5" strokeWidth={1.75} />
            <span className="mt-5 block text-[10px] uppercase tracking-[0.2em] text-bone/60">Found one?</span>
          ...
          <div className="rounded-2xl border border-forest/5 bg-bone p-4">
            ...
            <span className="mt-5 block text-[10px] uppercase tracking-[0.2em] text-moss/60">Nearby list</span>
        ...
        <section className="rounded-3xl border border-forest/5 bg-bone p-5 shadow-sm">
          <div className="flex items-end justify-between gap-4">
            <div>
              <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-moss/60">Challenges</span>
              <h2 className="mt-1 font-serif text-2xl font-bold">Collection goals</h2>
            </div>
            <span className="text-xs font-bold text-gold">{quests.filter((q) => q.progress >= q.total).length}/{quests.length}</span>
          </div>
          <div className="mt-4 grid gap-3">
            {quests.map((quest) => <Quest key={quest.title} {...quest} />)}
          </div>
        </section>
        <section>
          ...
          <div className="mt-3 space-y-3">
            {plants.map((p, i) => (
              <button key={p.id} onClick={() => go({ name: "species", id: p.id })} className="flex w-full items-center gap-4 rounded-2xl border border-forest/5 bg-bone p-3 text-left shadow-sm">
                <img src={p.image} alt={p.name} className="size-20 flex-shrink-0 rounded-xl object-cover" />
                <div className="min-w-0 flex-1">
                  <div className="mb-1 flex flex-wrap items-center gap-2">
                    <span className={"rounded px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-widest " + rarityClass[p.rarity]}>{p.rarity}</span>
                    <span className="inline-flex items-center gap-1 text-[10px] uppercase tracking-wider text-moss/60"><MapPin className="size-3" />{(0.2 + i * 0.3).toFixed(1)} mi</span>
                  </div>
...
function ProfileScreen({ profile, onProfileChange }: { profile: Profile; onProfileChange: (profile: Profile) => void }) {
  const fileRef = useRef<HTMLInputElement>(null);
  const collection = useCollection();
  const stats = collectionStats(collection);
  ...
  return (
    <>
      <header className="mobile-safe-top px-6 pb-6">
        <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-moss/60">Account</span>
      ...
      <main className="flex-1 space-y-5 px-6">
        <section className="rounded-3xl border border-forest/5 bg-bone p-5 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              <h2 className="truncate font-serif text-2xl font-bold">{displayName || "New naturalist"}</h2>
              <p className="mt-1 text-sm text-moss/70">{region || "No home region set"}</p>
              <p className="mt-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-gold">Tap photo to change</p>
            </div>
          </div>
          <input
            ref={fileRef}
        ...
        <section className="space-y-4 rounded-3xl border border-forest/5 bg-bone p-5 shadow-sm">
          <Input label="Display name" value={displayName} onChange={(value) => { setDisplayName(value); setSaved(false); }} placeholder="e.g. Ian" serif />
          <Input label="Home region" value={region} onChange={(value) => { setRegion(value); setSaved(false); }} placeholder="e.g. Singapore, Sydney" />
          <Select
            label="Experience"
            value={experience}
            onChange={(value) => { setExperience(value); setSaved(false); }}
            options={["New naturalist", "Garden hobbyist", "Field recorder", "Botany pro"]}
          />
          <button
            type="button"
          ...
          >
            <span>
              <span className="block text-sm font-semibold">Location-assisted discoveries</span>
              <span className="mt-1 block text-xs leading-relaxed text-moss/70">Use GPS for specimens and future nearby plant suggestions.</span>
            </span>
            <span className={"ml-4 h-7 w-12 rounded-full p-1 transition " + (shareLocation ? "bg-forest" : "bg-moss/20")}>
              <span className={"block size-5 rounded-full bg-bone transition " + (shareLocation ? "translate-x-5" : "")} />
            </span>
          </button>
          <button
            type="button"
    ...
    <>
      <main className="min-h-dvh bg-[#151812] px-5 pb-10 text-bone">
        <div className="mobile-safe-top flex items-center justify-between pb-5">
          <button onClick={() => go({ name: "herbarium" })} className="grid size-10 place-items-center rounded-full border border-bone/20 bg-bone/90 text-forest shadow-lg" aria-label="Back">
        ...
        <section className="overflow-hidden rounded-[2rem] border border-bone/20 bg-bone text-forest shadow-2xl">
          <div className="relative aspect-[4/5] overflow-hidden bg-white">
            ...
            <div className="absolute bottom-4 left-4 right-4 rounded-2xl bg-bone/95 p-4 shadow-xl backdrop-blur">
              <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-gold">Personal specimen</span>
              <h1 className="mt-1 font-serif text-3xl font-bold italic leading-tight">{specimen.name}</h1>
              {specimen.latin && <p className="mt-1 font-serif text-sm italic text-moss">{specimen.latin}</p>}
            </div>
          </div>
          <div className="p-5">
            <div className="grid grid-cols-2 gap-3">
        ...
        <section className="mt-5 rounded-[2rem] border border-bone/10 bg-bone/10 p-5 backdrop-blur-md">
          <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-bone/50">Field notes</span>
          {specimen.notes ? (
            <p className="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-bone/80">{specimen.notes}</p>
          ) : (
...
function SpecimenCardButton({ specimen, onClick }: { specimen: Specimen; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className="overflow-hidden rounded-2xl border border-forest/5 bg-bone text-left shadow-sm active:scale-[0.99]">
      <div className="relative aspect-[4/5] overflow-hidden bg-white">
        <img src={specimen.image} alt={specimen.name} className="h-full w-full object-cover" />
      ...
      <div className="p-3">
        ...
        <p className="mt-1 truncate text-[10px] italic text-moss/70">{specimen.latin || specimen.habitat || timeAgo(specimen.createdAt)}</p>
      </div>
    </button>
  );
}
...
function MissionStep({ index, title, done }: { index: string; title: string; done: boolean }) {
  return (
    <div className={"rounded-2xl border p-3 text-center " + (done ? "border-gold/30 bg-gold/10" : "border-forest/5 bg-sage")}>
      <span className="block text-[9px] font-bold text-gold">{index}</span>
      <span className="mt-1 block text-xs font-bold uppercase tracking-wider text-forest">{title}</span>
    </div>
  );
}
...
function MiniTrait({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl bg-sage p-3 text-center">
      <span className="block text-[8px] font-bold uppercase tracking-wider text-moss/60">{label}</span>
...
function SpeciesPage({ id, go }: { id: string; go: (screen: Screen) => void }) {
  ...
  if (!plant) return <EmptyDetail title="Species not found" onBack={() => go({ name: "discover" })} />;
  return (
    <>
      <DetailImage image={plant.image} title={plant.name} onBack={() => go({ name: "discover" })} cover>
        <div className={"rounded-full px-3 py-1.5 text-xs font-bold uppercase tracking-wider shadow-lg " + rarityClass[plant.rarity]}>{plant.rarity}</div>
      </DetailImage>
      <section className="relative z-10 -mt-8 flex-1 rounded-t-[3rem] bg-bone px-7 pb-12 pt-8 shadow-[0_-20px_40px_-15px_rgba(0,0,0,0.08)]">
        <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-moss/60">Field guide entry</span>
    ...
    <nav className="mobile-dock-float pointer-events-none fixed inset-x-0 bottom-0 z-40 flex justify-center px-3">
      <div className="liquid-glass pointer-events-auto grid w-full max-w-md grid-cols-4 items-center rounded-[1.55rem] p-1">
        {items.map(({ name, label, Icon }) => {
          ...
          return (
            <button key={name} type="button" onClick={() => go({ name })} className="relative z-10 flex min-w-0 flex-col items-center gap-0.5 rounded-2xl px-2 py-1.5 active:scale-95">
              <span className={"grid size-8 place-items-center rounded-xl transition " + (prominent ? "bg-forest/90 text-bone shadow-lg shadow-forest/20" : selected ? "bg-white/65 text-forest shadow-sm ring-1 ring-white/70" : "text-moss/55")}>
                <Icon className="size-[1.15rem]" strokeWidth={1.75} />
              </span>
              <span className={"max-w-full truncate text-[8px] font-semibold uppercase tracking-[0.12em] " + (selected ? "text-forest" : "text-moss/50")}>{label}</span>
            </button>
          );
        })}
      </div>
...
function PageHeader({ eyebrow, title, copy }: { eyebrow: string; title: string; copy: string }) {
  return (
    <header className="mobile-safe-top px-6 pb-6">
      <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-moss/60">{eyebrow}</span>
...
function Avatar({ profile, size }: { profile: Pick<Profile, "displayName" | "avatarImage">; size: "sm" | "lg" }) {
  const initials = profile.displayName
    ...
    .join("") || "BF";
  const sizeClass = size === "lg" ? "size-24 rounded-[1.45rem] text-2xl" : "size-10 rounded-full text-sm";
  ...
  return (
    <span className={"grid place-items-center bg-forest font-serif font-bold italic text-bone " + sizeClass}>
      {initials}
    </span>
  );
}
...
function Input({ label, value, onChange, placeholder, serif = false }: { label: string; value: string; onChange: (value: string) => void; placeholder: string; serif?: boolean }) {
  return (
    <label className="block">
      <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-moss/60">{label}</span>
      <input value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className={"mt-2 w-full rounded-2xl border border-forest/10 bg-bone px-4 py-4 placeholder:text-moss/40 focus:border-gold focus:outline-none " + (serif ? "font-serif text-base italic" : "text-sm")} />
    </label>
  );
}
...
function Select({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: string[] }) {
  return (
    <label className="block">
      <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-moss/60">{label}</span>
      <select value={value} onChange={(e) => onChange(e.target.value)} className="mt-2 w-full rounded-2xl border border-forest/10 bg-bone px-3 py-4 text-sm focus:border-gold focus:outline-none">
...
function Quest({ title, detail, progress, total }: { title: string; detail: string; progress: number; total: number }) {
  const done = progress >= total;
  return (
    <div className="rounded-2xl bg-sage p-3">
      <div className="flex items-start gap-3">
        <CheckCircle2 className={done ? "mt-0.5 size-5 text-gold" : "mt-0.5 size-5 text-moss/30"} strokeWidth={1.75} />
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-3">
            <h3 className="text-sm font-semibold">{title}</h3>
            <span className="text-[10px] font-bold text-moss/60">{progress}/{total}</span>
          </div>
          <p className="mt-1 text-xs text-moss/70">{detail}</p>
          <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-forest/5">
            <div className="h-full rounded-full bg-gold" style={{ width: `${(progress / total) * 100}%` }} />
          </div>
        </div>
      </div>
    </div>
...
function DetailImage({ image, title, onBack, cover = false, children }: { image: string; title: string; onBack: () => void; cover?: boolean; children?: React.ReactNode }) {
  return (
    <div className="relative bg-white">
      <img src={image} alt={title} className={"aspect-square w-full " + (cover ? "object-cover" : "object-contain p-8")} />
      <div className="absolute inset-x-0 top-0 flex items-start justify-between p-5">
        <button onClick={onBack} className="grid size-10 place-items-center rounded-full border border-forest/5 bg-bone/90 shadow-md backdrop-blur" aria-label="Back">
...
function InfoBox({ icon: Icon, label, value }: { icon: typeof Leaf; label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-forest/5 bg-bone p-4">
  ...
  return (
    <div className="flex items-center justify-between py-3.5">
      <div className="flex items-center gap-3">
        <Icon className="size-4 text-moss" strokeWidth={1.75} />
        <span className="text-[11px] font-semibold uppercase tracking-widest text-moss/70">{label}</span>
      </div>
  ...
  return (
    <main className="grid flex-1 place-items-center px-8 text-center">
      <div>
        ...
        <button onClick={onBack} className="mt-6 rounded-2xl bg-forest px-5 py-3 text-xs font-semibold uppercase tracking-wider text-bone">Go back</button>
      </div>
    </main>
  );
}
